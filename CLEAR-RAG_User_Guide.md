...
- The platform recognizes an **Evaluator** role (can create projects, run evaluations, and score answers), but role assignment after registration depends on how your organization manages the system. The current product does not expose a full self-service user management screen for changing roles.
...
- Uploaded files can be used as searchable evaluation content. URI-linked documents are stored as references, but in the current version they are not extracted into searchable retrieval text automatically.
...
**Why it is useful:** Uploaded files can be searched during evaluation, and URI entries help your team keep track of externally hosted reference documents.
...
Note: depending on the deployment, the app may show this state with a shorter label such as a ready or blocked badge instead of the exact wording below.
...
Shows each gate's pass/fail status and whether the run is currently ready or blocked for production use.
...
|---|---|---|
| Project | Name, system type, target users, description | Backend-supported; visible UI controls may vary by deployment |
| Source Document | Title, document type, version | Backend-supported; visible UI controls may vary by deployment |
| Test Question | Question text, question type, expected source | Backend-supported; visible UI controls may vary by deployment |
| Evaluation Run | Name, system version, notes | Backend-supported; visible UI controls may vary by deployment |
| CLEAR-RAG Scores | All five scores, reviewer notes, suggested improvement | Use the review/update workflow on the run page |
| Error Tags | Category, severity, notes, evidence reference | Backend-supported; visible UI controls may vary by deployment |

---

### 3. General Edit Process

1. Navigate to the record you want to change.
2. If your deployment shows an in-app edit control, use it. If no edit control is visible, ask your administrator or technical operator for help.
3. Update the information.
4. Save the changes.
5. The updated information appears immediately after a successful update.
...
Most records in CLEAR-RAG can be updated after creation, but the exact edit controls depend on the deployment. In the current app, the clearest in-product update path is the Evaluation Review workflow for scores. All edits are logged automatically in the Audit Trail.
...
|---|---|---|
| Project | Removes entire project and all related data (documents, questions, runs, scores, errors) | Admin role required; visible UI control may vary by deployment |
| Source Document | Removes document and its chunks/embeddings | Backend-supported; visible UI control may vary by deployment |
| Test Question | Removes question from the project | Backend-supported; visible UI control may vary by deployment |
| Evaluation Run | Removes run and all its outputs (chunks, answers, scores, errors) | Backend-supported; visible UI control may vary by deployment |
| Retrieved Chunk | Removes single retrieved chunk from a question result | Backend-supported; visible UI control may vary by deployment |
| Generated Answer | Removes single answer from a question result | Backend-supported; visible UI control may vary by deployment |
| CLEAR-RAG Evaluation | Removes score record for one answer | Backend-supported; visible UI control may vary by deployment |
| Error Tag | Removes specific error annotation | Backend-supported; visible UI control may vary by deployment |
...
3. Sarah uses her organization's admin process to have James assigned the **Evaluator** role.
